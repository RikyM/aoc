...
class Hand
  ...
  def value
    groups = @cards.group_by{|c| c}

    if groups.size == 1
      combs = groups.values[0].size.to_s
    else
      jollies = groups[JOLLY_VALUE] || []
      combs = groups.filter{|c,n| c != JOLLY_VALUE}.values.map(&:size).sort
      combs[-1] += jollies.size
      combs = combs.sort.join('')
    end
